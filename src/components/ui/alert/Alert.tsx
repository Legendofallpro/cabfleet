  ...
  return (
    <div className={cn(alertVariants({ variant }))}>
      <div className="flex items-start gap-3">
        <div className={cn("-mt-0.5", iconVariants({ variant }))}>
          {icons[variant!]}
        </div>
        <div>
          <h4 className="mb-1 text-sm font-semibold text-default">
            {title}
          </h4>
          ...
          {showLink && (
            <Link
              href={linkHref}
              className="mt-3 inline-block text-sm font-medium text-muted underline"
            >
              {linkText}
            </Link>
          )}
