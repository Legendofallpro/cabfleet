...
import { updateStaffAction } from "@/modules/staff/actions/staff.actions";
...
export function StaffEditForm({
  ...
  async function onSubmit(values: UpdateStaffFormValues) {
    const result = await updateStaffAction(values);
    if (!result.ok) {
      if (result.error.fieldErrors) {
        for (const [field, msgs] of Object.entries(result.error.fieldErrors)) {
          setError(field as keyof UpdateStaffFormValues, { message: msgs?.[0] });
        }
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <FormActions cancelHref="/staff" submitting={isSubmitting} />
    </form>
  );
}
